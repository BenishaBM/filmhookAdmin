///// the most most main code do not miss it ----------------------------------------------------------
...
//     return (
//         <div className="user-list-container min-h-screen bg-gradient-to-br  via-indigo-50 to-purple-50 px-4 py-8 sm:px-6 lg:px-8">
...
const UserList = () => {
    ...
    const [selectKey, setSelectKey] = useState(Date.now()); // For forcing select to re-render
    
    // Calculate total pages based on total records and page size
    const totalPages = Math.ceil(totalRecords / pageSize) || 3; // Default to 3 pages if calculation is 0
    ...
    useEffect(() => {
        ...
        if (valueofuser) {
            if (Array.isArray(valueofuser)) {
                setUserData(valueofuser);
                setTotalRecords(valueofuser.length * 3); // Estimate total for pagination
            } else if (valueofuser.data && Array.isArray(valueofuser.data)) {
                setUserData(valueofuser.data);
                // If API returns total count, use it
                setTotalRecords(valueofuser.totalRecords || valueofuser.data.length * 3);
            } else {
                console.log("Unknown data structure, raw valueofuser:", valueofuser);
                setUserData([]);
            }
    ...
    const handlePageSizeChange = (e) => {
        setPageSize(Number(e.target.value));
        setCurrentPage(1);
        // Force select to re-render with new value
    ...
    // Translation object (can be replaced with proper i18n)
    const t = {
        ...
        <div className="user-list-container min-h-screen bg-gradient-to-br via-indigo-50 to-purple-50 px-4 py-8 sm:px-6 lg:px-8">
            <div className="max-w-6xl mx-auto">
                ...
                <div className="bg-white rounded-xl shadow-xl overflow-hidden mb-10 transition-all duration-300 hover:shadow-2xl">
                    ...
                    {/* Footer with Pagination - Using the provided template */}
                        ...
                        <div className="flex items-center gap-4">
                            <div className="w-18 h-8 flex items-center justify-center rounded-full border border-black">
                                <select
                                    ...
                                    onChange={handlePageSizeChange}
                                    className="w-full h-full text-sm border-0 bg-transparent focus:ring-0"
                                >
                            ...
                            <button
                                className={`px-4 py-1 text-sm rounded-full border border-black ${
                                    currentPage === 1
