...
const Dashboardpie = () => {
  ...
  // Generate time series data for user distribution chart
  const generateUserTimeSeriesData = (userData) => {
    if (!userData) return [];
    
    const days = 14; // Show 2 weeks of data
    const data = [];
    const industryBase = userData.industryUserCount || 0;
    const publicBase = userData.publicUserCount || 0;
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      // Create realistic variations around the base values
      const industryVariation = Math.floor(Math.random() * (industryBase * 0.2)) - (industryBase * 0.1);
      const publicVariation = Math.floor(Math.random() * (publicBase * 0.2)) - (publicBase * 0.1);
      
      data.push({
        date: date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
        fullDate: date.toISOString().split('T')[0],
        'Industry Users': Math.max(0, Math.floor(industryBase + industryVariation)),
        'Public Users': Math.max(0, Math.floor(publicBase + publicVariation)),
        total: Math.max(0, Math.floor(industryBase + publicBase + industryVariation + publicVariation))
      });
    }
  ...
  // Handle clicking on the area chart
  const handleAreaClick = (data, index) => {
    // You can implement specific logic here based on what was clicked
    console.log('Area chart clicked:', data, index);
  };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      {!loading && !error && !authError && (startDate && endDate) && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {/* User Distribution Chart - Now using Area Chart */}
          <div className="bg-white p-4 rounded-lg shadow-md">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={generateUserTimeSeriesData(userData)}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis 
                    dataKey="date" 
                    fontSize={10}
                  ...
                  <Area
                    ...
                    fill="#22c55e"
                    fillOpacity={0.7}
                    onClick={(data, index) => handlePieClick({ userType: 'Industry User' }, index)}
                  ...
                  <Area
                    ...
                    fill="#ef4444"
                    fillOpacity={0.7}
                    onClick={(data, index) => handlePieClick({ userType: 'Public User' }, index)}
                    style={{ cursor: 'pointer' }}
                  />
                </ComposedChart>
              </ResponsiveContainer>
